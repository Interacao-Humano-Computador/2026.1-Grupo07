# Reunião 03

## Introdução

Este documento tem como objetivo registrar as deliberações, decisões e o acompanhamento das atividades realizadas durante as reuniões do grupo de projeto de Interação Humano-Computador (IHC). O registro sistemático destas atas visa garantir a transparência do processo de design e a rastreabilidade das decisões técnicas e gerenciais tomadas ao longo do semestre[^1].

## 1. Identificação
| Campo | Informação |
|------|------------|
| Projeto | Sigla do Projeto - Nome do Projeto |
| Data | 12/04/2026 |
| Local | Plataforma Teams |
| Redator | Maria Laura Regis |

### Horários

- *Início (Previsto):* -
- *Início (Realizado):* -
- *Término (Realizado):* -

---

## 2. Participantes 
| Nº | Convocados | Presença (Presente/Ausente) | Área / Empresa |
|----|------------|-----------------------------|----------------|
| 1 | Maria Laura Regis | Presente | UNB - Engenharia de Software |
| 2 | Phelipe Amâncio | Presente | UNB - Engenharia de Software |
| 3 | Vitor Evangelista | Presente | UNB - Engenharia de Software |
| 4 | Hugo | Presente | UNB - Engenharia de Software |
| 5 | Ingrid | Ausente | UNB - Engenharia de Software |
| 6 | Thaiza | Presente | UNB - Engenharia de Software |

---

## 3. Pauta
| Nº | Descrição |
|----|------------|
| 1 | Discussão dos sites avaliados |
| 2 | Escolha do site a ser utilizado no projeto |

---

## 4. Pendências Anteriores
Nenhuma pendência registrada.

---

## 5. Assuntos Tratados
| Nº | Descrição | Tipo[^2] |
|----|------------|--------|
| 1 | Discussão sobre os sites avaliados pela equipe | Discussão |
| 2 | Escolha do site selecionado em conjunto | Definição |

### Detalhamento da Distribuição de Tarefas

- Documentação dos sites avaliados → *Vitor e Thaiza*
- Documentação do site selecionado → *Vitor e Thaiza*
- Publicação no GitHub Pages → *Hugo*
- Elaboração da ata da reunião → *Maria Laura*

---

## 6. Compromissos
| Nº | Compromisso | Responsável | Data |
|----|--------------|-------------|------|
| 1 | Documentar os sites avaliados | Vitor e Thaiza | Próxima reunião |
| 2 | Documentar o site selecionado | Vitor e Thaiza | Próxima reunião |
| 3 | Publicar conteúdo no GitHub Pages | Hugo | Próxima reunião |
| 4 | Elaborar a ata da reunião | Maria Laura | Concluído |

---

## 7. Próxima Reunião
- *Data/Previsão:* 12/04/2026  
- *Objetivo:* Gravação da apresentação do projeto
___

## Link da gravação: <a href="https://www.youtube.com/watch?v=9zEJGohv-KQ/">(link de acesso)</a>

### Bibliografia Consultada

* BRASIL. Ministério da Ciência, Tecnologia e Inovações. **Modelo de Ata de Reunião**. Brasília: MCTI, [202-?]. Disponível em: [Modelo Ata](https://ps.mctic.gov.br/MCTI-PS/guidances/examples/Ata_Reuniao_35E290FE.html?nodeId=e8d6222d). Acesso em: 11 abr. 2026. [Acesso direto ao pdf](../ModeloAta.pdf).
## Histórico de Versão
| Versão | Data | Descrição | Autor | Revisor |
| :--- | :--- | :--- | :--- | :--- |
| 1.0 | 11/04/2026 | Criação do documento  |[Ingrid Alves](https://github.com/alvesingrid) | [Maria Laura Regis](https://github.com/Maria-Laura-Regis) |
| 1.1 | 22/04/2026 | Adição do historico de versão| [Ingrid Alves](https://github.com/alvesingrid) |[Hugo Freitas Silva](https://github.com/HugoFreitass) |
| 1.2 | 26/04/2026 | Adequação ao feedback do professor | [Philipe Amancio](https://github.com/Phill-Chill) |  |


[^1]: Estrutura adaptada do padrão de Atas do MCTI.
[^2]: **Definições dos Tipos de Itens de Pauta:**

    - **Apresentação:** Exposição de assunto que não depende de avaliação ou aprovação (Ex: Leitura de atas anteriores).

    - **Decisão:** Execução aprovada que não impacta custo ou prazo (Ex: Ajuste em requisito de baixa complexidade).

    - **Definição:** Mudança aprovada pelo solicitante que **causa impacto** em prazo e custo (Ex: Imposição de nova funcionalidade pelo gestor).

    - **Solicitação:** Item que depende de análise de viabilidade e estudo prévio para ser aprovado ou descartado.

    - **Pendência:** Dúvida não resolvida que depende de terceiros ou discussões futuras.